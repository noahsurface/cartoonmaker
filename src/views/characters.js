...
const LAYER_DEFS = [
  { key: 'body', title: 'Body', help: 'Pick one idle pose, and any number of frames to cycle through while the character is moving.' },
...
function blankCharacter(name) {
  return {
    ...
    name,
    createdAt: Date.now(),
    layers: { body: blankLayer(), mouth: blankLayer(), eyes: blankLayer() },
  };
}
...
export async function render(root, params) {
  ...
  async function renderList() {
    ...
    for (const character of characters) {
      const idleId = character.layers?.body?.roles?.idle;
      const idleFrame = character.layers?.body?.frames?.find((f) => f.id === idleId);
  ...
  async function renderEditor() {
    ...
    const character = await getRecord('characters', editingCharacterId);
    if (!character) {
  ...
  async function buildLayerBlock(character, def) {
    ...
    async function refreshLayerBlock() {
      ...
      rolesEl.innerHTML = '';
      if (def.key === 'body') {
        rolesEl.appendChild(buildSingleRolePicker(layer, 'idle', 'Idle frame', character));
        rolesEl.appendChild(buildOrderedRolePicker(layer, 'cycle', 'Move cycle (click frames in play order)', character));
      } else if (def.key === 'mouth') {
        rolesEl.appendChild(buildSingleRolePicker(layer, 'silent', 'Silent frame', character));
        rolesEl.appendChild(buildOrderedRolePicker(layer, 'talk', 'Talk loop (click frames in play order)', character));
    ...
    function buildSingleRolePicker(layer, roleKey, label) {
    ...
    function buildOrderedRolePicker(layer, roleKey) {
      const wrap = document.createElement('div');
      wrap.className = 'role-row';
