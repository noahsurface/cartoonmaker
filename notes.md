...
- `src/seed.js` imports the bundled sample art from `assets/samples/` into the asset store under fixed ids the first time the app runs (idempotent — safe to call on every boot), plus a starter character ("Bruce") and starter scene, so the app is usable with zero uploads.
